...
def test_discriminator02(Discriminator):
    batch_size = 50
    conv_dim=10
    img_size = 64
    D = Discriminator(conv_dim, img_size)

    # create random image input
    x = torch.from_numpy(np.random.randint(1, size=(batch_size, 3, img_size, img_size))*2 -1).float()
...
def test_discriminator03(Discriminator, layer_num):
    batch_size = 50
    conv_dim=10
    img_size = 64
    #layer_num = 4
    D = Discriminator(conv_dim, img_size, layer_num)
...
def test_generator02(Generator):
    batch_size = 50
    z_size = 25
    conv_dim=10
    img_size = 64
    G = Generator(z_size, conv_dim, img_size)
    ...
    correct_output_size = (batch_size, 3, img_size, img_size)
    assert_condition = output.size() == correct_output_size
    assert_message = 'Wrong output size. Expected type {}. Got type {}'.format(correct_output_size, output.size())
    assert_test.test(assert_condition, assert_message)
...
def test_generator03(Generator, layer_num):
    batch_size = 50
    z_size = 25
    conv_dim=10
    img_size = 64
    #layer_num = 4
    G = Generator(z_size, conv_dim, img_size, layer_num)
